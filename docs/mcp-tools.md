...
- `available`: source text is available to this account with `get_strategy` and `includeSourceCode: true`. This does not prove the text is a complete standalone Pine strategy.
...
Response note: `sourceAvailability` is `available`, `no`, or `private`. When `includeSourceCode` is true and source is available, the response includes `sourceCode`.
